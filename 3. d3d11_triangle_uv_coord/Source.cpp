...
/* global pointer to texture2d */
ID3D11Texture2D* texture2d_ptr;

/* global pointer to sampler state (uv stuff) */
ID3D11SamplerState* sampler_state_ptr;

/* global pointer to shader resource view (uv stuff) */
ID3D11ShaderResourceView* texture_view_ptr;
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE prevInstance, PWSTR pCmdLine, int nCmdShow)
{
	...
	/* 1. We start by defining an array of vertex points AND ALSO put in our UV coordinates! */
	float vertex_data_array[] =
	{
		0.f, 0.5f, 0.f, 0.0f, 0.0f,			// top
	...
	while (!closed)
	{
		...
		// 3. Set the Output Merger (tell our Output Merger to use our Render Target)
