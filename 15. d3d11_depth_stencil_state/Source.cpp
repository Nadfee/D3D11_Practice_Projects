...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE prevInstance, PWSTR pCmdLine, int nCmdShow)
{
	...
	sr_data2.pSysMem = vertex_data_array;

	hr = device_ptr->CreateBuffer(&vertex_buffer_desc, &sr_data2, &vertex_buffer_ptr2);
	assert(SUCCEEDED(hr));
	...
	// How do you access the Depth Buffer in a 2nd pass? Do you use SRV? It gives errors..
	...
	while (!closed)
	{
		...
		XMMATRIX m_Projection = XMMatrixPerspectiveFovLH(fovAngle, aspectRatio, 0.1f, 15.f);
		...
		m_World = XMMatrixTranslation(2.f, 0.f, 5.f);
		worldViewProj = m_World * camView * m_Projection;

		Matrixes* mats2 = (Matrixes*)(cbufMov2Subres.pData);
		mats2->m_WorldViewProj = XMMatrixTranspose(worldViewProj);
		mats2->m_World = XMMatrixTranspose(m_World);
...
HWND n_CreateWindow(HINSTANCE hInstance, const wchar_t* CLASS_NAME)
{
	HWND hwnd = CreateWindowEx(
		...
		CW_USEDEFAULT, CW_USEDEFAULT, 1280, 720,	// x, y, width, height
		NULL,				// parent window
		NULL,				// menu
