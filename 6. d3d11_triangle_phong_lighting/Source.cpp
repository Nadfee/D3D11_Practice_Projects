...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE prevInstance, PWSTR pCmdLine, int nCmdShow)
{
	...
	{
		0.0f, 0.5f, 1.f, 0.f, 0.f, -1.f,		// top
		0.5f, -0.5f, 1.f, 0.f, 0.f, -1.f,		// bottom right
		0.0f, -0.5f, 1.f, 0.f, 0.f, -1.f,		// bottom left
		0.1f, 0.5f, 1.f, 0.f, 0.f, -1.f,
		0.75f, 0.5f, 1.f, 0.f, 0.f, -1.f,
		0.6f, -0.5f, 1.f, 0.f, 0.f, -1.f
	};

	UINT vertex_stride = 6 * sizeof(float);		// (what is stride?)
	...
	buffer_desc.StructureByteStride = 0;					// 0 (?)


	hr = device_ptr->CreateBuffer(&buffer_desc, NULL, &constant_buffer_ptr);
	...
	while (!closed)
	{
		...
		const_buffer* my_buf = (const_buffer*)(mapped_subresource.pData);
		my_buf->position = XMFLOAT3(sinf(time), 0.f, 0.f);
		my_buf->padding = 1.f;
		...
		device_context_ptr->PSSetConstantBuffers(0, 1, &constant_buffer_ptr);	// note that the cbuffer components will be able to be used in Pixel Shader!
