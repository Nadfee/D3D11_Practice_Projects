...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE prevInstance, PWSTR pCmdLine, int nCmdShow)
{
	...
	while (!closed)
	{
		...
		std::string time = "FPS: " + std::to_string((int)(1.l / frameTime)) + " FrameTime: " + std::to_string(frameTime * 1000.l) + "ms";
		SetWindowTextA(hwnd, time.c_str());
		...
		XMMATRIX m_World = XMMatrixTranslation(0.f, 0.f, 5.f);
		...
		D3D11_MAPPED_SUBRESOURCE cbufSubres;
		device_context_ptr->Map(const_buf_mov, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &cbufSubres);

		Matrixes* mats = (Matrixes*)(cbufSubres.pData);
