...
float translationOffset2[3] = { 0.f, 0.f, 1.5f };
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE prevInstance, PWSTR pCmdLine, int nCmdShow)
{
	...
	{
		// pos, normal, uv
		-0.75f, 1.f, 0.f, 0.f, 0.f, -1.f, 0.0f, 0.f,
		0.5f, -1.f, 0.f, 0.f, 0.f, -1.f, 1.f, 1.f,
		-0.75f, -1.f, 0.f, 0.f, 0.f, -1.f, 0.f, 1.f,

		//-0.5f, 1.f, 0.f, 0.f, 0.f, -1.f, 0.f, 0.f,
		//0.75f, 1.f, 0.f, 0.f, 0.f, -1.f, 1.f, 0.f,
		//0.75f, -1.f, 0.f, 0.f, 0.f, -1.f, 1.f, 1.f
	};
	...
	ID3D11Buffer* vertex_buffer_ptr2;
	//float vertex_data_array2[] =
	//{
	//	// pos, normal, uv
	//	-0.5f, 1.f, 0.f, 0.f, 0.f, -1.f, 0.0f, 1.f, 0.f,
	//	0.75f, 1.f, 0.f, 0.f, 0.f, -1.f, 0.0f, 1.f, 0.f,
	//	0.75f, -1.f, 0.f, 0.f, 0.f, -1.f, 0.0f, 1.f, 0.f,
	//};
	...
	sr_data2.pSysMem = vertex_data_array2;
	UINT vertex_count2 = sizeof(vertex_data_array2) / vertex_stride;

	D3D11_BUFFER_DESC vertex_buffer_desc2 = vertex_buffer_desc;
	vertex_buffer_desc2.ByteWidth = sizeof(vertex_data_array2);

	hr = device_ptr->CreateBuffer(&vertex_buffer_desc2, &sr_data2, &vertex_buffer_ptr2);
	assert(SUCCEEDED(hr));
	...
	// Make Texture2D
	GetClientRect(hwnd, &winRect);		// NOTE DIFFERENCE BETWEEN WindowRect and ClientRect!
	...
	depthTextDesc.Width = 1024;
	depthTextDesc.Height = 1024;
	depthTextDesc.MipLevels = 1;
	depthTextDesc.ArraySize = 1;
	depthTextDesc.Format = DXGI_FORMAT_R32_TYPELESS;		// DXGI_FORMAT_D24_UNORM_S8_UINT
	...
	// Make a Shader Resource View to access depth buffer in second pass!
	...
	hr = device_ptr->CreateShaderResourceView(depthText, &srvDesc, &depthSRV);
	assert(SUCCEEDED(hr));

	// 2nd Pass rendering (Create and compile shaders)
	...
	hr = device_ptr->CreateSamplerState(&sampler_desc, &sampler_state);
	assert(SUCCEEDED(hr));


	// Make another Depth Stencil View (for 2nd pass!)
	ID3D11Texture2D* depthText2;
	D3D11_TEXTURE2D_DESC depthTextDesc2;
	depthTextDesc2.Width = winRect.right - winRect.left;
	depthTextDesc2.Height = winRect.bottom - winRect.top;
	depthTextDesc2.MipLevels = 1;
	depthTextDesc2.ArraySize = 1;
	depthTextDesc2.Format = DXGI_FORMAT_R32_TYPELESS;		// DXGI_FORMAT_D24_UNORM_S8_UINT
	depthTextDesc2.SampleDesc.Count = 1;
	depthTextDesc2.SampleDesc.Quality = 0;
	depthTextDesc2.Usage = D3D11_USAGE_DEFAULT;
	depthTextDesc2.BindFlags = D3D11_BIND_DEPTH_STENCIL;
	depthTextDesc2.CPUAccessFlags = 0;
	depthTextDesc2.MiscFlags = 0;

	hr = device_ptr->CreateTexture2D(&depthTextDesc2, NULL, &depthText2);
	assert(SUCCEEDED(hr));

	// Make Depth Stencil View
	ID3D11DepthStencilView* dsv_ptr2;
	D3D11_DEPTH_STENCIL_VIEW_DESC dsvDesc2 = {};
	dsvDesc2.Format = DXGI_FORMAT_D32_FLOAT;		// DXGI_FORMAT_D24_UNORM_S8_UINT
	dsvDesc2.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2D;
	dsvDesc2.Texture2D.MipSlice = 0;

	hr = device_ptr->CreateDepthStencilView(depthText2, &dsvDesc2, &dsv_ptr2);
	assert(SUCCEEDED(hr));
	...
	while (!closed)
	{
		...
		// Clear background
		float background_color[] = { 1.f, 0.f, 1.f, 1.f };

		// Calculate World, View and Projection matrix

		XMMATRIX m_World = XMMatrixTranslation(translationOffset[0], translationOffset[1], translationOffset[2]) * XMMatrixScaling(5.f, 1.f, 5.f);
		m_World *= XMMatrixRotationX(rotationOffset[0]);

		if (keepUpdate)
			UpdateCamera();		// camView/m_View

		float fovAngle = (M_PI / 180.f) * (70.f);
		float aspectRatio = ((float)winRect.right - (float)winRect.left) / ((float)winRect.bottom - (float)winRect.top);

		XMMATRIX m_Projection = XMMatrixPerspectiveFovLH(fovAngle, aspectRatio, 0.5f, 15.f);
		...
		// For light view
		FXMVECTOR lightPosition = XMVectorSet(0.f, 2.f, -1.f, 1.f);
		FXMVECTOR lookLocation = XMVectorSet(0.f, 0.f, 1.f, 1.f);
		XMMATRIX m_LView = XMMatrixLookAtLH(lightPosition, lookLocation, XMVectorSet(0.f, 1.f, 0.f, 0.f));

		XMMATRIX m_LProjection = XMMatrixPerspectiveFovLH(fovAngle, aspectRatio, 0.5f, 15.f);
		//XMMATRIX m_LProjection = XMMatrixOrthographicLH(10.f, 10.f, 0.5f, 15.f);		// Kinda more like a directional light!

		XMMATRIX LWorldViewProj = m_World * m_LView * m_LProjection;

		mats->m_LWorldViewProj = XMMatrixTranspose(LWorldViewProj);
		...
		lights->light_pos = XMFLOAT3(XMVectorGetX(lightPosition), XMVectorGetY(lightPosition), XMVectorGetZ(lightPosition));
		...
		D3D11_MAPPED_SUBRESOURCE cbufMov2Subres;
		device_context_ptr->Map(const_buf_mov2, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &cbufMov2Subres);

		XMMATRIX another_m_World = XMMatrixTranslation(translationOffset2[0], translationOffset2[1], translationOffset2[2]);
		worldViewProj = another_m_World * camView * m_Projection;

		Matrixes* mats2 = (Matrixes*)(cbufMov2Subres.pData);
		mats2->m_WorldViewProj = XMMatrixTranspose(worldViewProj);
		mats2->m_World = XMMatrixTranspose(another_m_World);

		XMMATRIX another_LWorldViewProj = another_m_World * m_LView * m_LProjection;
		mats2->m_LWorldViewProj = XMMatrixTranspose(another_LWorldViewProj);

		device_context_ptr->Unmap(const_buf_mov2, NULL);



		//*************************

		// testing output of first pass
		...
		//device_context_ptr->IASetVertexBuffers(0, 1, &vertex_buffer_ptr2, &vertex_stride, &vertex_offset);
		...
		//// Go!
		//device_context_ptr->Draw(vertex_count2, 0);

		//// Second tri
		//device_context_ptr->VSSetConstantBuffers(0, 1, &const_buf_mov2);
		...
		// Viewport dimensions to the shadow map width and height!
		// https://learnopengl.com/Advanced-Lighting/Shadows/Shadow-Mapping
		// "What is important to note here are the calls to glViewport"
		// "Because shadow maps often have a different resolution compared to what we originally render the scene in 
		...
		device_context_ptr->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
		device_context_ptr->ClearDepthStencilView(dsv_ptr, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);	// DEPTH STENCIL
		device_context_ptr->OMSetDepthStencilState(dsState, 0);	// DEPTH STENCIL
		device_context_ptr->IASetInputLayout(input_layout_ptr);
		device_context_ptr->IASetVertexBuffers(0, 1, &vertex_buffer_ptr2, &vertex_stride, &vertex_offset);
		...
		device_context_ptr->OMSetRenderTargets(0, 0, dsv_ptr);		// only populating our depth buffer!
		...
		device_context_ptr->RSSetViewports(1, &viewportNew);



		device_context_ptr->ClearDepthStencilView(dsv_ptr2, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);	// fix :)
		device_context_ptr->OMSetRenderTargets(1, &render_target_view_ptr, dsv_ptr2);	// stencil and depth check is not used! things will still dependent on draw call order
																					// we have to give NULL so we can use the depth values as texture
		device_context_ptr->ClearRenderTargetView(render_target_view_ptr, background_color);
		device_context_ptr->PSSetShaderResources(0, 1, &depthSRV);		
		...
		device_context_ptr->PSSetSamplers(0, 1, &sampler_state);	// maybe buffer info (SRV) (texture) is zeroed because of code below???
		//device_context_ptr->ClearDepthStencilView(dsv_ptr2, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 0.0f, 0);	// This clearing fixed the glitch (triangle background color floating around)
		...
		ImGui::DragFloat3("Translation Offset X/Y/Z", translationOffset, 0.125f, -10.f, 100.f);
		ImGui::DragFloat3("Rotation Offset X/Y/Z", rotationOffset, 0.125f, -6.14f, 6.14f);

		ImGui::DragFloat3("Another Translation Offset X/Y/Z", translationOffset2, 0.125f, -10.f, 100.f);
...
void n_CreateConstantBuffer()
{
	...
	hr = device_ptr->CreateBuffer(&buf_desc_mov, NULL, &const_buf_mov2);
	assert(SUCCEEDED(hr) && const_buf_mov2);
}
