...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE prevInstance, PWSTR pCmdLine, int nCmdShow)
{
	...
	UINT vertex_stride = 9 * sizeof(float);
	...
	// Make rasterizer state (turn off back culling)
	...
	hr = device_ptr->CreateRasterizerState(&rast_desc, &rast_state);
	assert(SUCCEEDED(hr));
	...
	while (!closed)
	{
		...
		// Clear color
		float background_color[] = { 0.f, 1.f, 0.f, 1.f };
		float background_color2[] = { 1.f, 0.f, 1.f, 1.f };
		...
		device_context_ptr->RSSetState(rast_state);
		...
		device_context_ptr->OMSetRenderTargets(BUFFER_COUNT, rtv_ptrs, NULL);
